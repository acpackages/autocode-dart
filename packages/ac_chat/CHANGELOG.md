...
- Deprecated `PlayListItem` in favor of `AcPlaylistTrack`.
- Deprecated `AcVoiceRecordingListPlayer` in favor of `AcVoiceRecordingAttachmentPlaylist`.
- Deprecated `AcVoiceRecordingLoading` as it is no longer used.
- Deprecated `AcVoiceRecordingPlayer` in favor of `AcVoiceRecordingAttachment`.
- Deprecated `AcVoiceRecordingSlider` in favor of `StreamAudioWaveformSlider`.
- Deprecated `AcVoiceRecordingAttachmentBuilder` in favor of `AcVoiceRecordingAttachmentPlaylistBuilder`.
...
- Added several new widgets to enhance the AI assistant features.
    - `AcMessageView` to show AI assistant messages with streaming animation.
    - `AcAITypingIndicatorView` to show AI typing indicator.
...
- Added `customAttachmentBuilders` parameter for `AcAttachmentWidgetBuilder.defaultBuilders`.
...
- Added `AcVoiceRecordingAttachmentBuilder`, for displaying voice recording attachments in the chat.
...
- [[#1804]](https://github.com/GetStream/stream-chat-flutter/issues/1804) Fixed
  `AcAttachmentWidgetBuilder`` is not accessible in 7.0.0.
...
- Added support
  for `AcChannelAvatar.ownSpaceAvatarBuilder`, `AcChannelAvatar.oneToOneAvatarBuilder` and
  `AcChannelAvatar.groupAvatarBuilder` to override the default avatar
...
- Added `attachmentActionsModalBuilder` parameter to `StreamMessageWidget` that allows to
  customize `AcAttachmentActionsModal`.
...
- [[#1347]](https://github.com/GetStream/stream-chat-flutter/issues/1347) `onReply` not working
  in `AcAttachmentActionsModal` which is used by `AcImageAttachment` and `StreamImageGroup`.
...
- Added `AcAutocomplete` widget for autocomplete triggers in `StreamMessageInput`.
...
- Breaking changes 🚧
    - `AcImageAttachment.size` has been converted from type `Size` to type `BoxConstraints`
    - `AcFileAttachment.size` has been converted from type `Size` to type `BoxConstraints`
    - `AcGiphyAttachment.size` has been converted from type `Size` to type `BoxConstraints`
    - `AcVideoAttachment.size` has been converted from type `Size` to type `BoxConstraints`
...
- Fixed AcChannelHeader and StreamThreadHeader subtitle alignment.
...
- Removed `isOwner` condition from `ChannelBottomSheet` and `AcChannelInfoBottomSheet` for
  delete option tile.
...
- Deprecated `ChannelPreview` in favor of `StreamChannelListTile`.
- Deprecated `ChannelAvatar` in favor of `AcChannelAvatar`.
- Deprecated `ChannelName` in favor of `AcChannelName`.
