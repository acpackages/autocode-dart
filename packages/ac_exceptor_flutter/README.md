...
Add `ac_exceptor_flutter` to your `pubspec.yaml`:

```yaml
dependencies:
  ac_exceptor_flutter:
    path: ../ac_exceptor_flutter
```
...
import 'package:ac_exceptor_flutter/ac_exceptor_flutter.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: Scaffold(
        body: Center(child: Text('ac_exceptor_flutter Active')),
      ),
    );
  }
}
