...
class AcMirrorsAggregatingBuilder implements Builder {
  ...
  Future<void> build(BuildStep buildStep) async {
    ...
    final initialLibraries = <LibraryElement>[];

    await for (final inputId in buildStep.findAssets(Glob('lib/**.dart'))) {
      if (!await buildStep.resolver.isLibrary(inputId)) continue;
      initialLibraries.add(await buildStep.resolver.libraryFor(inputId));
    }

    log("Found ${initialLibraries.length} top-level libraries in this package. Discovering all reachable files...");
    for (final library in initialLibraries) {
      _collectAllReachableLibraries(library, allReachableLibraries);
    }

    log("Found ${allReachableLibraries.length} total libraries to scan (including imports and exports).");
  ...
  void _collectAllReachableLibraries(LibraryElement library, Set<LibraryElement> visited) {
    if(visited.contains(library)) return;
    log("    -> Discovering library: ${library.source.uri}");
    visited.add(library);

    for (final importedLibrary in library.importedLibraries) {
      _collectAllReachableLibraries(importedLibrary, visited);
    }
    for (final exportedLibrary in library.exportedLibraries) {
      _collectAllReachableLibraries(exportedLibrary, visited);
    }
  }
  ...
  void generateDeclarationMirrors(StringBuffer buffer, ClassElement classEl) {
    ...
    for (final declaration in getAllDeclarations(classEl).where((d) => !d.isSynthetic)) {
      ...
      if (declaration is ConstructorElement) {
        ...
        buffer.writeln("  @override Symbol get simpleName => const Symbol('${declaration.name}');");
  ...
  void generateClassMirror(StringBuffer buffer, ClassElement classEl) {
    ...
    if (!classEl.isAbstract) {
      log("  -> Found ${classEl.constructors.length} constructors for '$className'.");
      for(final member in classEl.constructors) {
    ...
    if (classEl.isAbstract) {
      ...
      for(final ctor in classEl.constructors.where((c) => c.isPublic)) {
    ...
    for(final member in instanceMembers.values.where((m) => !m.isPrivate)) {
      if (member is FieldElement) {
        ...
        final params = List.generate(member.parameters.length, (i) => 'positionalArgs[$i]').join(', ');
        if (member.returnType.isVoid) {
          buffer.writeln("      case const Symbol('${member.name}'): instance.${member.name}($params); break;");
        } else {
          buffer.writeln("      case const Symbol('${member.name}'): return instance.${member.name}($params);");
        }
      }
    }
    buffer.writeln('      default: final memberNameString = memberName.toString().split(\'\\"\')[1]; throw UnimplementedError("Cannot invoke \\"\$memberNameString\\" on \\"$className\\".");');
    buffer.writeln('    }');
    buffer.writeln('  }');
