...
class AcWeb {
  ...
  Future<AcWebResponse> handleWebRequest(
    ...
    ClosureMirror closureMirror =
        reflect(routeDefinition.handler) as ClosureMirror;
    MethodMirror functionMirror = closureMirror.function;
    for (final parameter in functionMirror.parameters) {
      ...
      if (parameter.type.reflectedType == AcWebRequest) {
        ...
        for (var meta in parameter.metadata) {
          ...
          final key = paramName;
          if (instance is AcWebValueFromPath &&
              request.pathParameters.containsKey(key)) {
            args.add(request.pathParameters[key]);
            valueSet = true;
          } else if (instance is AcWebValueFromQuery &&
              request.get.containsKey(key)) {
            args.add(request.get[key]);
            valueSet = true;
          } else if (instance is AcWebValueFromForm &&
              request.post.containsKey(key)) {
            args.add(request.post[key]);
            valueSet = true;
          } else if (instance is AcWebValueFromHeader &&
              request.headers.containsKey(key)) {
            args.add(request.headers[key]);
            valueSet = true;
          } else if (instance is AcWebValueFromCookie &&
    ...
    if (routeDefinition.controller != null) {
      var controllerMirror = reflectClass(
        routeDefinition.controller!,
      ).newInstance(Symbol(''), []);
      var methodMirror =
          controllerMirror.type.instanceMembers[Symbol(
  ...
  AcWeb route({
    ...
    var acDocRoute =
        acApiDocRoute ??
        _getRouteDocFromHandlerReflection(
          handler: handler,
          acApiDocRoute: null,
        );
