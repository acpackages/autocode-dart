...
class AcMysqlDao extends AcBaseSqlDao {
  ...
  Future<AcSqlDaoResult> getDatabaseStoredProcedures() async {
    ...
    try {
      ...
      for (final row in results.rows) {
        result.rows.add({
          AcDDStoredProcedure.KEY_STORED_PROCEDURE_NAME: row.colByName(
            'ROUTINE_NAME',
          ),
        });
      }
  ...
  Future<AcSqlDaoResult> getDatabaseViews() async {
    ...
    try {
      ...
      for (final row in results.rows) {
        result.rows.add({AcDDView.KEY_VIEW_NAME: row.colByName('TABLE_NAME')});
      }
      result.setSuccess();
    } catch (ex, stack) {
      result.setException(exception: ex, stackTrace: stack);
    ...
    try {
      ...
      for (final row in results.rows) {
        ...
        if (row.colByName('Default') != null) {
          properties[AcEnumDDColumnProperty.DEFAULT_VALUE] = row.colByName(
            'Default',
          );
        }
        result.rows.add({
